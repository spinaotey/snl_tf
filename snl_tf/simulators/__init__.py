import numpy as np
from snl_tf import pdfs
import snl_tf.util.misc as umisc
import snl_tf.util.io as uio
...
def gaussian_synthetic_likelihood(px, sim_model, log=True, n_sims=100, rng=np.random):
    ...
    ps, xs, one_datapoint =umisc.prepare_cond_input(px, float)
    Ls = []
    ...
    def load(self, n_samples):
        ...
        try:
            if self.n_files == 0:
                ...
                self.ps, data = uio.load(self.file_pattern.format(0))
            ...
            while self.ps.shape[0] < n_samples:
                ...
                ps, data = uio.load(self.file_pattern.format(self.n_files))
