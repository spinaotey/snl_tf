...
class WeightedSequentialNeuralLikelihood:
    """
    Trains a likelihood model using weighted posterior MCMC sampling to guide simulations.
    """

    def __init__(self, prior, nominal_samples, reweighter):

        self.prior = prior
        self.nominal_samples = nominal_samples
        self.n_nominal = len(nominal_samples)
    ...
    def learn_likelihood(self, obs_xs, model, trainer, sess, n_samples, n_rounds, train_on_all=True, 
        ...
        log_posterior = lambda t: l_posterior_fw(self.prior,model,sess,t,obs_xs)
        sampler = mcmc.SliceSampler(self.prior.gen(), log_posterior, thin=thin)
        ...
        for i in range(rounds,n_rounds):
            ...
            # run new batch of simulations
            logger.write('reweighting data... ')
            ps, xs, ws = simulators.rw_data(proposal.gen, self.reweighter,
                                            self.nominal_samples, n_samples, rng=rng)
            xs = xs[:,:model.n_outputs]
            logger.write('done\n')
            self.all_ps.append(ps)
            self.all_xs.append(xs)
            ...
            if save_models:
                trainer.train(sess,(ps,xs),ws,max_iterations=max_iterations, batch_size=batch_size,p_val=p_val,
                              early_stopping=early_stopping, check_every_N=check_every_N, 
                              saver_name=model_sufix+str(i),show_log=True)
                np.savez(model_sufix+'data.npz',ps=np.concatenate(self.all_ps),xs=np.concatenate(self.all_xs))
            else:
...
def l_posterior_fw(prior,model,sess,t,obs_xs):
    """
    Computes log posterior probability of paramters `t` for a weighted
    tensorflow model given observed data points `obs_xs`.
    :param prior: distribution to evaluate prior parameter density
    :param model: the model to evaluate the posterior
    :param sess: tensorflow session where the model is run
    :param obs_xs: observed data to evaluate posterior
    ...
    """
    vt = np.repeat([t],obs_xs.shape[0],axis=0)
    w = np.ones([len(obs_xs),1])
    return sess.run(model.L,feed_dict={model.input:vt,model.y:obs_xs,model.weights:w}).sum() + prior.eval(t)